...
def state_enc(cfg):
	...
	return (nn.Sequential(nn.Linear(cfg.state_dim, cfg.enc_dim), nn.ELU(),
...
class RandomShiftsAug(nn.Module):
	...
	def __init__(self, cfg):
		super().__init__()
		self.pad = int(cfg.img_size/21) # maintain same padding ratio as in original implementation
...
class Episode(object):
	...
	def __init__(self, cfg, init_obs, init_state=None):
		...
		self.obs = torch.empty((cfg.episode_length+1, *init_obs.shape), dtype=torch.uint8, device=self.device)
		self.obs[0] = torch.tensor(init_obs, dtype=torch.uint8, device=self.device)
		self.state = torch.empty((cfg.episode_length+1, *init_state.shape), dtype=torch.float32, device=self.device)
		self.state[0] = torch.tensor(init_state, dtype=torch.float32, device=self.device)
	...
	def from_trajectory(cls, cfg, obs, states, action, reward, done=None):
		...
		episode.state[1:] = torch.tensor(states[1:], dtype=episode.state.dtype, device=episode.device)
...
class ReplayBuffer(object):
	...
	def __init__(self, cfg):
		...
		self._obs = torch.empty((self.capacity+1, *obs_shape), dtype=torch.uint8, device=self.device)
		self._last_obs = torch.empty((self.capacity//cfg.episode_length, *cfg.obs_shape), dtype=torch.uint8, device=self.device)
		...
		self._state = torch.empty((self.capacity, self._state_dim), dtype=torch.float32, device=self.device)
		self._last_state = torch.empty((self.capacity//cfg.episode_length, self._state_dim), dtype=torch.float32, device=self.device)
	...
	def _get_obs(self, arr, idxs):
		obs = torch.empty((self.cfg.batch_size, self.cfg.frame_stack*3, *arr.shape[-2:]), dtype=arr.dtype, device=torch.device('cuda'))
	...
	def sample(self):
		...
		probs /= probs.sum()
		total = len(probs)
		idxs = torch.from_numpy(np.random.choice(total, self.cfg.batch_size, p=probs.cpu().numpy(), replace=True)).to(self.device)
		weights = (total * probs[idxs]) ** (-self.cfg.per_beta)
		weights /= weights.max()
		obs = self._get_obs(self._obs, idxs) if self.cfg.frame_stack > 1 else self._obs[idxs].cuda(non_blocking=True)
		...
		for t in range(self.cfg.horizon+1):
			_idxs = idxs + t
			next_obs[t] = self._get_obs(self._obs, _idxs+1) if self.cfg.frame_stack > 1 else self._obs[_idxs+1].cuda(non_blocking=True)
