...
class AdroitWrapper(gym.Wrapper):
	...
	def _get_state_obs(self, obs):
		if self.cfg.task == 'adroit-door':
			...
			qp = self.env.data.qpos.ravel()
			desired_orien = (self.env.data.site_xpos[self.env.tar_t_sid] - self.env.data.site_xpos[self.env.tar_b_sid])/self.env.tar_length
	...
	def step(self, action):
		...
		reward = float(info['success']) - 1.
		return self._stacked_obs(), reward, False, info

	def render(self, mode='rgb_array', width=None, height=None, camera_id=None):
		return np.flip(self.env.env.sim.render(mode='offscreen', width=width, height=height, camera_name=self.camera_name), axis=0)
