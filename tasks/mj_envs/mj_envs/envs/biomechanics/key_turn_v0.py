...
class KeyTurnFixedEnvV0(BaseV0):
    ...
    def get_reward_dict(self, obs_dict):
        IF_approach_dist = np.abs(
            np.linalg.norm(self.obs_dict["IFtip_approach"], axis=-1) - 0.040
        )
        TH_approach_dist = np.abs(
            np.linalg.norm(self.obs_dict["THtip_approach"], axis=-1) - 0.040
        )

        key_qvel = (
            obs_dict["key_qvel"][:, :, 0]
            if obs_dict["key_qvel"].ndim == 3
            else obs_dict["key_qvel"][0]
        )
        far_th = 0.07
        ...
        rwd_dict = collections.OrderedDict(
            (
                ...
                (
                    "penalty",
                    (IF_approach_dist > far_th / 2) + (TH_approach_dist > far_th / 2),
                ),
                # Must keys
                ("sparse", key_qvel),
