...
class BaseV0(env_base.MujocoEnv):
    ...
    def _setup(
        ...
        **kwargs,
        ...
        if self.muscle_condition == "weakness":
            for mus_idx in self.which_muscles:
                self.sim.model.actuator_gainprm[mus_idx, 2] = (
                    self.which_gain_muscles[mus_idx]
                    * sim.model.actuator_gainprm[mus_idx, 2]
                )
        ...
        if self.sim.model.na:
            assert (
                self.normalize_act is False
            ), "Explict action remapping is performed for envs with muscle actuator. Turn act_normalized to False to remove default linear remapping to action limits"
    ...
    def step(self, a):
        ...
        if self.muscle_condition == "fatigue":
            for mus_idx in range(self.sim.model.actuator_gainprm.shape[0]):
                self.f_load[mus_idx].append(
                    self.sim.data.actuator_moment[mus_idx, 1].copy()
                )
