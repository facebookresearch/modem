...
class OptiTrack(hardwareBase):
    ...
    def __init__(
        self, ip: str, port: int = 5000, packet_size: int = 36, cache_maxsize: int = 0
    ):
    ...
    def connect(self):
        if self._OPTI_CLIENT is None:
            ...
            if _USE_UDP:
                self._OPTI_CLIENT = socket.socket(
            ...
            if self._sensor_cache:
                print(
                    "Initializing sensor cache. Polling {} samples".format(
                        self.sensor_cache_maxsize
                    )
                )
    ...
    def read_sensor(self):
        ...
        if len(data) != self.packet_size:  # if not all data read. Read leftovers
            ...
            print(
                "Partial packet of length {} found at t={:3.3f}. Reading \
                additional {} bytes".format(
                    n_read, self.data_float[0], n_left
                )
            )
        ...
        self.sensor_data = {
            "time": self.data_float[0:1],
            "id": self.data_float[1:2],
            "pos": self.data_float[2:5],
            "quat": self.data_float[5:9],
        }
    ...
    def __repr__(self):
        return "ET:{:03.3f}, id:{:2d}, x:{:+03.3f}, y:{:+03.3f}, z:{:+03.3f}".format(
            self.data_float[0],
            int(self.data_float[1]),
            self.data_float[2],
            self.data_float[3],
            self.data_float[4],
        )
...
def get_args():
    ...
    parser.add_argument(
        ...
        default="zyx.cs.washington.edu",
    )  # 169.254.163.96
    parser.add_argument(
        "-p", "--port", type=int, help="Port to use (> 1024)", default=5000
    )
    parser.add_argument(
        "-n", "--nbytes", type=int, help="Size of packets being exchanged", default=36
    )  # [t, id, x, y, z, q0, q1, q2, q3, q4]
    parser.add_argument(
        "-v", "--verbose", type=bool, help="print data stream", default=False
    )
    ...
    while oc.okay():
        ...
        if args.verbose:
            if data is None:
                ...
                print(
                    "T:{:03.3f}, id:{:2d}, x:{:+03.3f}, y:{:+03.3f}, z:{:+03.3f}".format(
                        data[0], int(data[1]), data[2], data[3], data[4]
                    )
                )
            time.sleep(0.1)
    oc.close()
